...
class FilterSubSubCategeoryVC: CategoryFilterBaseVC {
    
//    var objFilter: FilterCategory = FilterCategory()
    var oldFilter: FilterCategory? = FilterCategory()

    @IBOutlet weak var lblNavTitle: UILabel!
    @IBOutlet weak var btnDone: UIButton!
    
    var objSubCat: SubCategory?
...
//        self.btnDone.isHidden = self.subCategories.first(where: { $0.isSelected }) == nil
    ...
    override func viewDidLoad() {
        ...
        oldFilter = self.navigation?.objVc.objFilter
        
        lblNavTitle.text = /objSubCat?.name
...
extension FilterSubSubCategeoryVC{
    
    func webServiceSubCategoriesListing () {
        
        let objR = API.SubCategoryListing(supplierId: FilterCategory.supplierId, categoryId: objSubCat?.subCategoryId)
...
extension FilterSubSubCategeoryVC {
    ...
    func configureTableViewInitialization(){
        ...
        tableViewDataSource = TableViewDataSource(
            items: subCategories, height: UITableView.automaticDimension , tableView: tableView, cellIdentifier: CellIdentifiers.FilterSubcategoryTableViewCell ,
            configureCellBlock: {
                (cell, item) in
...
//        }
        ...
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: FilterNotification), object: self.navigation?.objVc)
    }
}
