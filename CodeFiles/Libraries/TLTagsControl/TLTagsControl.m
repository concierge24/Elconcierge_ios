...
- (void)commonInit {
    ...
    _tagInputField.layer.cornerRadius = 5;
    ...
    _tagInputField.font = [UIFont fontWithName:@"ProximaNova-light" size:14];
...
- (void)layoutSubviews {
    ...
    if (_mode == TLTagsControlModeEdit) {
        ...
        if (self.frame.size.width - _tagInputField.frame.origin.x > 100) {
            frame.size.width = self.frame.size.width - frame.origin.x - 12;
        } else {
            frame.size.width = 100;
        }
        _tagInputField.frame = frame;
    } else {
    ...
    _tagInputField.placeholder = (_tagPlaceholder == nil) ? @"tag" : _tagPlaceholder;
}
...
- (void)addTag:(NSString *)tag {
    ...
    if (contentSize.width > self.frame.size.width) {
        if (_mode == TLTagsControlModeEdit) {
            offset.x = _tagInputField.frame.origin.x + _tagInputField.frame.size.width - self.frame.size.width;
        } else {
            UIView *lastTag = tagSubviews_.lastObject;
...
- (void)reloadTagSubviews {
    ...
    for (NSString *tag in _tags) {
        ...
        if (_mode == TLTagsControlModeEdit) {
            UIButton *deleteTagButton = [[UIButton alloc] initWithFrame:_tagInputField.frame];
            CGRect buttonFrame = deleteTagButton.frame;
            [deleteTagButton.titleLabel setFont:_tagInputField.font];
            ...
            buttonFrame.size.height = _tagInputField.frame.size.height;
            ...
            [deleteTagButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        ...
        //        tagFrame.origin.y = tagFrame.origin.y + 12;
        //        tagFrame.size.height = tagFrame.size.height - 12;
    ...
    if (_mode == TLTagsControlModeEdit) {
        ...
        CGRect frame = _tagInputField.frame;
        if (tagSubviews_.count == 0) {
            frame.origin.x = 7;
        } else {
            UIView *view = tagSubviews_.lastObject;
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if (textField.text.length > 0) {
        ...
        //        textField.text = @"";
        //        [self addTag:tag];
        NSArray * arrTags = [textField.text componentsSeparatedByString:@" "];
