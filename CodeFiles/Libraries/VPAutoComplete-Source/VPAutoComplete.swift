...
class VPAutoComplete: UIView {
    ...
    func changeHeightForCount(count: Int){
        ...
        self.frame =  CGRect(x: 0, y: 40, width: onView!.frame.size.width, height: onView!.frame.size.height)
...
extension String {
    ...
    func width(font: UIFont) -> CGFloat {
        let label =  UILabel(frame: CGRect(x: 0, y: 0, width: 10, height: font.pointSize+4))
