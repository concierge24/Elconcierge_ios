...
public class PageControl: UIControl {
    ...
    public var currentPage: Int {
        ...
        set {
            setCurrentPage(currentPage: CGFloat(newValue), animated: true)
        }
    }
    ...
    public func setCurrentPage(currentPage: CGFloat, animated: Bool = false) {
        ...
        updateCurrentPageDisplayWithAnimation(animated: animated)
        updateAccessibility()
    }
    ...
    public var numberOfPages: Int = 0 {
        didSet {
            ...
            setCurrentPage(currentPage: _currentPage, animated: false)
            updateAccessibility()
        }
    }
    ...
    private func didInit() {
        ...
        updateCurrentPageDisplayWithAnimation(animated: false)
        updateAccessibility()
    }
}
...
private extension PageControl {
    func updateCurrentPageDisplayWithAnimation(animated: Bool = true) {
        let frame = frameForPageIndicator(page: _currentPage, forNumberOfPages: numberOfPages)
...
extension PageControl {
    public override func layoutSubviews() {
        ...
        for (index, view) in pageIndicatorContainerView.subviews.enumerated() {
            view.frame = frameForPageIndicator(page: CGFloat(index), forNumberOfPages: numberOfPages)
        }
...
extension PageControl {
    public override func sizeThatFits(_ size: CGSize) -> CGSize {
        if numberOfPages == 0 || hidesForSinglePage && numberOfPages == 1 {
            ...
            return sizeForNumberOfPages(pageCount: numberOfPages)
        }
    }
    ...
    public override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first, isEnabled == true {
            ...
            if !defersCurrentPageDisplay {
                updateCurrentPageDisplayWithAnimation(animated: true)
                updateAccessibility()
            }
