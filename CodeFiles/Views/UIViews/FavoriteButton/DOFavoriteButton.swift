...
public class DOFavoriteButton: UIButton {
    ...
    private func addTargets() {
        ...
        // add target
        //===============
        self.addTarget(self, action: #selector(DOFavoriteButton.touchDown(sender:)), for: .touchDown)
        self.addTarget(self, action: #selector(DOFavoriteButton.touchUpInside(sender:)), for: .touchUpInside)
        self.addTarget(self, action: #selector(DOFavoriteButton.touchDragExit(sender:)), for: .touchDragExit)
        self.addTarget(self, action: #selector(DOFavoriteButton.touchDragEnter(sender:)), for: .touchDragEnter)
        self.addTarget(self, action: #selector(DOFavoriteButton.touchCancel(sender:)), for: .touchCancel)
    }
    ...
    public func deselect() {
        ...
        UIView.animate(withDuration: 0.75, delay: 0.0, usingSpringWithDamping: 0.4, initialSpringVelocity: 0.0, options: .curveEaseInOut, animations: {
            weak var weak : DOFavoriteButton? = self
            weak?.transform = CGAffineTransform.identity
            }, completion: nil)
    }
}
