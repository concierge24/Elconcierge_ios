...
class HeaderView: UIView {
    ...
    override func layoutSubviews() {
        ...
        toggleButton.addTarget(self, action: #selector(HeaderView.toggle(sender:)), for: UIControl.Event.touchUpInside)
    ...
    @objc func toggle(sender:AnyObject){
        ...
        if self.tableView!.sectionOpen != section! {
            self.delegate?.headerViewOpen(section: section!)
        } else if self.tableView!.sectionOpen != NSNotFound {
            self.delegate?.headerViewClose(section: self.tableView!.sectionOpen)
        }
    }
}
