...
- (id) initWithWidth:(float)width withHeightForEachRow:(float)height originPointX:(CGFloat)originPointX  originPointY : (CGFloat)originPointY withOptions:(NSArray*)options {
    
    CGRect frame ;
    
    if (options.count >= 5)
    {
        frame=CGRectMake(originPointX, originPointY, width, (height*5)+12);
    }else{
        
        frame=CGRectMake(originPointX , originPointY, width, (height*options.count)+12);
    }
...
- (id) initWithWidth:(float)width withHeightForEachRow:(float)height originPoint:(CGPoint)originPoint withOptions:(NSArray*)options{
    ...
    if (options.count >= 5)
    {
        frame=CGRectMake(originPoint.x, originPoint.y, width, (height*5)+12);
    }else{
        frame=CGRectMake(originPoint.x, originPoint.y, width, (height*options.count)+12);
    }
